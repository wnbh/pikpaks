<template>
  <n-layout has-sider v-resize @resize="handleResize" position="absolute">
    <n-layout-sider 
...
import { IconFiles, IconClockUp, IconUpload,IconVideo, IconPhoto, IconShare, IconTrash,IconUsersPlus, IconAdjustments, IconCircleX, IconLogout } from "@tabler/icons-vue";
...
import { useRoute, useRouter } from 'vue-router';
  ...
  const goRoute = (key:string, item:MenuOption) => {
    console.log(item)
    if (item.key == "upload") {
      router.push('/' + item.key + JSON.parse(window.localStorage.getItem('upload_path') || '[]'))
    } else {
  ...
  const postCode = () => {
    http.post('https://api-drive.mypikpak.com/vip/v1/order/activation-code', {
      ...
      .then(res => {
        window.$message.success('兑换成功' , { duration: 1000 })
        getAbout()
      })
      .catch(error => {
        console.log(error)
        ...
        onPositiveClick: () => {
          http.post('https://user.mypikpak.com/v1/auth/revoke', {})
            .then(res => {
              ...
              window.$message.success('已退出' , { duration: 1000 })
              router.push('/login')
            })
            .catch(error => {
              console.log(error)
