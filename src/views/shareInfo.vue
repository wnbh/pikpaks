  ...
  <n-spin :show="loading " description="获取中">
    <div class="share-info-page" >
      <div class="header" v-if="shareInfo?.share_status === 'OK'">
        ...
        <div class="action">
          <n-button type="primary" @click="restore">{{checkedRowKeys.length ? '转存' + (checkedRowKeys.length) + '项' : '转存全部'}}</n-button>
        </div>
      </div>
      ...
      <template v-else-if="shareInfo?.share_status === 'OK'">
        <n-data-table v-model:checked-row-keys="checkedRowKeys" :row-key="row => row.id" :data="fileList" size="small" :columns="columns" :bordered="false" :scroll-x="600" max-height="calc(100vh - 150px)"></n-data-table>
...
const restore = () => {
  ...
  http.post('https://api-drive.mypikpak.com/drive/v1/share/restore', {
    ...
    .then(res => {
      window.$message.success('转存成功，可能需要等待一段时间' , { duration: 1000 })
...
.n-data-table-td.created_time,.n-data-table-th.created_time {
